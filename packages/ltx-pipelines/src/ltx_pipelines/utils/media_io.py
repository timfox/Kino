...
def get_videostream_fps(path: str) -> float:
    fps, _, _, _ = get_videostream_metadata(path)
    return fps
...
def decode_video_by_frame(
    ...
    """Decode up to *frame_cap* RGB frames as ``(1, H, W, C)`` uint8 tensors."""
    yield from decode_video_from_file(path, frame_cap=frame_cap, device=device)
...
def decode_video_from_file(
    ...
    *,
    ...
    """Decode RGB frames as ``(1, H, W, C)`` uint8 tensors.

    Either pass *frame_cap* (fixed number of frames from the start) or use
    *start_time* / *max_duration* for a time window (used by video latent helpers).
    """
    container = av.open(path)
    try:
        video_stream = next(s for s in container.streams if s.type == "video")
        ...
        if start_time > 0:
            seek_ts = int(start_time / time_base)
            container.seek(seek_ts, any_frame=False, backward=True, stream=video_stream)

        yielded = 0
        for frame in container.decode(video_stream):
            if frame.pts is not None:
                t = float(frame.pts * time_base)
                if t + 1e-6 < start_time:
                    continue
                if end_time is not None and t > end_time + 1e-3:
                    break
...
def load_image_and_preprocess(
    ...
    """Load a single image, optional H.264 recompression, resize, and normalize for VAE encode."""
    return load_image_conditioning(image_path, height, width, dtype, device, crf=crf)


def video_preprocess(
    frame_gen: Iterator[torch.Tensor],
    height: int,
    width: int,
    dtype: torch.dtype,
    device: torch.device,
) -> torch.Tensor:
    """Resize + normalize each decoded frame and concatenate along the frame axis (VAE layout)."""
    ...
    for f in frame_gen:
        ...
        result = frame if result is None else torch.cat([result, frame], dim=2)
    if result is None:
        raise ValueError("video_preprocess: received zero frames from the input generator")
    return result
