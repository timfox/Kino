"""Runnable evaluation smoke for gaze_head (arXiv stub)."""
...
def _NUMPY_FALLBACK(cfg: Any) -> dict[str, Any]:
    import numpy as np
    gaze = np.array([0.1, 0.0])
    head = np.array([0.05, 0.02])
    coord = float(np.linalg.norm(gaze - head))
    return {"paper": "arXiv:2605.25810", "gaze_head_offset": round(coord, 4)}
