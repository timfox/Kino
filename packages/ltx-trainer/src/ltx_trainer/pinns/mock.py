"""Runnable evaluation smoke for pinns (arXiv stub)."""
...
def _NUMPY_FALLBACK(cfg: Any) -> dict[str, Any]:
    ...
    dst = src + np.array([0.05, 0.02])
    err = float(np.linalg.norm(dst - src, axis=1).mean())
    return {"paper": "arXiv:2605.25947", "reprojection_error": round(err, 4)}
