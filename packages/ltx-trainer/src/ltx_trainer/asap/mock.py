"""Runnable evaluation smoke for asap (arXiv stub)."""
...
def _NUMPY_FALLBACK(cfg: Any) -> dict[str, Any]:
    import numpy as np
    v_pred = np.ones((2, 4, 4, 4))
    v_tgt = np.zeros_like(v_pred)
    mask = np.zeros_like(v_pred)
    mask[:, :, :2, :2] = 1.0
    alpha = getattr(cfg, "localized_alpha", getattr(cfg, "anatomy_mask_alpha", 10.0))
    loss = float(((1.0 + alpha * mask) * (v_pred - v_tgt) ** 2).mean())
    return {"paper": "arXiv:2605.25759", "localized_fm_loss": round(loss, 4)}
