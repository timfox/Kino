"""Runnable evaluation smoke for evident (arXiv stub)."""
...
def _NUMPY_FALLBACK(cfg: Any) -> dict[str, Any]:
    import numpy as np
    gate = 1.0 / (1.0 + np.exp(-np.array([0.5, -0.2, 1.1])))
    return {"paper": "arXiv:2605.26104", "gate_mean": round(float(gate.mean()), 4)}
