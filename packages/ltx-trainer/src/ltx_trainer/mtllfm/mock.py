"""Runnable evaluation smoke for mtllfm (arXiv stub)."""
...
def _NUMPY_FALLBACK(cfg: Any) -> dict[str, Any]:
    import numpy as np
    scores = np.array([0.1, 0.9, 0.3, 0.85])
    peak = int(scores.argmax())
    return {"paper": "arXiv:2605.25409", "laughter_peak_frame": peak, "peak_score": round(float(scores[peak]), 4)}
