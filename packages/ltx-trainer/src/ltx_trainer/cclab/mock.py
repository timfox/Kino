"""Runnable evaluation smoke for cclab (arXiv stub)."""
...
def _NUMPY_FALLBACK(cfg: Any) -> dict[str, Any]:
    tp, loss, rtt, rtt_min, bmax = 10.0, 0.01, 0.05, 0.04, 100.0
    dt = max(0.0, rtt - rtt_min)
    delay_factor = 1.0 if cfg.delay_margin_gamma * rtt_min >= rtt else cfg.delay_margin_gamma * rtt_min / rtt
    reward = tp - cfg.loss_penalty_lambda * loss / bmax * delay_factor
    return {"paper": "arXiv:2605.21915", "cc_reward": round(reward, 4), "queuing_delay": round(dt, 4)}
