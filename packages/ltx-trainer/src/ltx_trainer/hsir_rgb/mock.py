"""Runnable evaluation smoke for hsir_rgb (arXiv stub)."""
...
def _NUMPY_FALLBACK(cfg: Any) -> dict[str, Any]:
    import numpy as np
    rgb = np.linspace(0, 1, 16, dtype=np.float64)
    noise = np.random.default_rng(0).standard_normal(16) * 0.05
    return {"paper": "arXiv:2605.24769", "denoise_mse": round(float(((rgb - (rgb + noise)) ** 2).mean()), 6)}
