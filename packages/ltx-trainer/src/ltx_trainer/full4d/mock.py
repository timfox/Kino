"""Runnable evaluation smoke for full4d (arXiv stub)."""
...
def _NUMPY_FALLBACK(cfg: Any) -> dict[str, Any]:
    import numpy as np
    pts = np.random.default_rng(0).standard_normal((32, 3))
    spread = float(np.linalg.norm(pts.std(axis=0)))
    return {"paper": "arXiv:2605.25500", "gaussian_spread": round(spread, 4)}
