"""Runnable evaluation smoke for anyscene (arXiv stub)."""
...
def _NUMPY_FALLBACK(cfg: Any) -> dict[str, Any]:
    import numpy as np
    occ = np.zeros((8, 8), dtype=np.float64)
    occ[2:6, 3:7] = 1.0
    bev_density = float(occ.mean())
    return {"paper": "arXiv:2605.26113", "bev_occupancy_density": round(bev_density, 4)}
