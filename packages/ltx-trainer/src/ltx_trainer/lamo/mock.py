"""Runnable evaluation smoke for lamo (arXiv stub)."""
...
def _NUMPY_FALLBACK(cfg: Any) -> dict[str, Any]:
    import numpy as np
    z = np.arange(24, dtype=np.float64).reshape(2, 3, 2, 2)
    delta = z[:, 1:] - z[:, :-1]
    mu = delta.mean(axis=(2, 3))
    return {"paper": "arXiv:2605.23878", "macro_drift_channels": int(mu.shape[1]), "tau": getattr(cfg, "latent_tau", 2)}
