...
class BasicAVTransformerBlock(torch.nn.Module):
    ...
    def forward(  # noqa: PLR0915
        ...
        if run_vx:
            ...
            # Sampler/attention kernels can promote activations to fp32 while
            # native LTX checkpoints keep FFN weights in bf16. Cast only at
            # the Linear boundary to support both resident and streamed modes.
            vx = vx + self.ff(vx_scaled.to(dtype=self.ff.net[0].proj.weight.dtype)) * vgate_mlp
        ...
        if run_ax:
            ...
            ax = ax + self.audio_ff(ax_scaled.to(dtype=self.audio_ff.net[0].proj.weight.dtype)) * agate_mlp
