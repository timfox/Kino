...
class Modality:
    ...
    Attributes:
        ...
        av_cross_attention_mask: Optional additive mask for audio→video cross-attention
            when this modality is the **query** side (video queries audio keys).
            Shape ``(B, T_video, T_audio)`` with blocked positions at ``finfo.min``.
    """
    ...
    av_cross_attention_mask: torch.Tensor | None = None
