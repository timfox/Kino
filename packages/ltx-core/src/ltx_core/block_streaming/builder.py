...
def expand_ltx_checkpoint_safetensors_paths(checkpoint_path: str) -> tuple[str, ...]:
    """Resolve safetensors shard paths for builders, native folds, and HF sharded exports."""
    path = Path(checkpoint_path).expanduser().resolve()
    if path.is_dir():
        manifest_path = path / "native_manifest.json"
        if manifest_path.is_file():
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            parts = manifest.get("parts") if isinstance(manifest, dict) else None
            if isinstance(parts, list) and parts:
                return tuple(str((path / part).resolve()) for part in parts)
        hits = sorted(path.glob("*.safetensors"))
        if hits:
            return tuple(str(item) for item in hits)
        return (str(path),)

    if not path.is_file():
        return (str(path),)

    match = _HF_SHARD_FILENAME_RE.match(path.name)
    if not match:
        return (str(path),)

    base, _, total_s = match.group(1), match.group(2), match.group(3)
    try:
        total = int(total_s)
    except ValueError:
        return (str(path),)

    pattern = f"{base}-*-of-{total_s}.safetensors"
    hits = sorted(path.parent.glob(pattern), key=lambda item: item.name)
    if len(hits) == total:
        out = tuple(str(item) for item in hits)
        if len(out) > 1:
            logger.info(
